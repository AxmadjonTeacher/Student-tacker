...
interface StudentTableProps {
  ...
  setEngMathSubSubject?: (sub: 'ENG' | 'MATH') => void;
  grantSubject?: 'ENG' | 'MATH';
  setGrantSubject?: (sub: 'ENG' | 'MATH') => void;
  onExportExcelRegister?: (exportFn: (() => void) | null) => void;
}
...
const StudentTable: React.FC<StudentTableProps> = ({ 
  ...
  useEffect(() => {
    if (authRole === 'teacher') {
      ...
      if (tSubject === 'ENG' || tSubject === 'MATH') {
        if (setGrantSubject) setGrantSubject(tSubject);
        if (setEngMathSubSubject) setEngMathSubSubject(tSubject);
      }
    }
  }, [authRole, setGrantSubject, setEngMathSubSubject]);
  ...
  const filteredEngMathStudents = useMemo(() => {
    return students.filter(student => matchesGradeRange(student.className, engMathGradeRange));
  }, [students, engMathGradeRange]);
  ...
  useEffect(() => {
    if (!onExportExcelRegister) return;
    
    let exportFn: (() => void) | null = null;
    if (activeSubject === 'DETAILS') {
      exportFn = () => handleExportCredentialsToExcel(sortedStudents);
    } else if (activeSubject === 'ENG_MATH') {
      exportFn = () => handleExportEngMathToExcel(filteredEngMathStudents, engMathSubSubject);
    } else if (activeSubject === 'ALL' || activeSubject === 'PRIMARY') {
      exportFn = () => handleExportAllToExcel(sortedStudents);
    } else if (activeSubject === 'GRANT') {
      exportFn = () => handleExportAllToExcel(students);
    }
    
    onExportExcelRegister(exportFn);
    return () => {
      onExportExcelRegister(null);
    };
    ...
    <>
          ...
          <>
            {activeSubject === 'ALL' || activeSubject === 'PRIMARY' ? (
              ...
              filteredEngMathStudents.forEach(student => {
                const rawTeacher = engMathSubSubject === 'ENG' ? student.teacher : student.mathTeacher;
                const tName = rawTeacher?.trim();
                // Filter out students with unassigned/empty teachers
                if (!tName) return; 
              ...
              const detailsGroupsMap: { [cls: string]: Student[] } = {};
              sortedStudents.forEach(student => {
                const cls = student.className || 'Sinf kiritilmagan';
                if (!detailsGroupsMap[cls]) {
                  detailsGroupsMap[cls] = [];
                }
                detailsGroupsMap[cls].push(student);
              });
              
              const detailsSortedClasses = Object.keys(detailsGroupsMap).sort((a, b) => {
              ...
              return (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem', marginBottom: '2rem', width: '100%' }}>
                  {detailsSortedClasses.map(clsName => {
                    const classStudents = detailsGroupsMap[clsName];
                    return (
                      <div 
                        key={clsName}
                        className="table-card-container"
                      ...
                      >
                        ...
                        <div className="table-header-row" style={{
                          ...
                          <div style={{ fontWeight: 900, color: 'var(--text-primary)', fontSize: '0.85rem', letterSpacing: '0.04em', borderRight: '1px solid var(--border-color)', display: 'flex', alignItems: 'center', padding: '0.9rem 0' }}>
                            Sinf: {clsName.toUpperCase()} ({classStudents.length} ta o'quvchi)
                          </div>
      ...
      {Object.keys(unsavedChanges).length > 0 && (
        <div style={{
          ...
          backdropFilter: 'var(--backdrop-blur-md)',
          WebkitBackdropFilter: 'var(--backdrop-blur-md)',
          border: '1.5px solid var(--accent-hero)',
          boxShadow: 'var(--glass-shadow-soft), inset 0 1px 0 var(--border-highlight)',
          padding: '1rem 2rem',
          borderRadius: '9999px',
          display: 'flex',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
            <div style={{
              width: '10px', height: '10px', borderRadius: '50%', background: 'var(--accent-hero)',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--accent-hero)' }}>
                <RotateCw size={20} />
                <h3 style={{ margin: 0, fontSize: '1.25rem', fontWeight: 800, letterSpacing: '-0.02em', color: 'var(--text-primary)' }}>
            ...
            <p style={{ fontSize: '0.85rem', color: 'var(--text-secondary)', lineHeight: 1.5, marginBottom: '1.5rem', fontWeight: 500 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.85rem', marginBottom: '1.5rem' }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', cursor: 'pointer', fontSize: '0.9rem', fontWeight: 600, color: 'var(--text-primary)' }}>
                <input
                  ...
                  style={{ width: '18px', height: '18px', accentColor: 'var(--accent-hero)', cursor: 'pointer' }}
                />
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              <select
                ...
                style={{
                  width: '100%', padding: '0.75rem 1rem', borderRadius: '9999px', border: '1px solid var(--border-subtle)',
                  fontSize: '0.9rem', fontWeight: 600, outline: 'none', color: 'var(--text-primary)', cursor: 'pointer',
                  background: 'var(--bg-card-hover)'
                }}
              >
                <option value="class" style={{ background: 'var(--bg-card)', color: 'var(--text-primary)' }}>Faqat ushbu sinf o'quvchilari uchun</option>
                <option value="all" style={{ background: 'var(--bg-card)', color: 'var(--text-primary)' }}>Barcha sinf o'quvchilari uchun</option>
              </select>
            </div>

            {/* Warning Box */}
            <div style={{
              background: 'rgba(239, 68, 68, 0.08)', border: '1px solid rgba(239, 68, 68, 0.2)', borderRadius: '20px',
              padding: '1rem', marginBottom: '2rem', display: 'flex', gap: '0.75rem', alignItems: 'flex-start'
            }}>
              <span style={{ fontSize: '1.2rem', lineHeight: 1 }}>⚠️</span>
              <div>
                <span style={{ display: 'block', fontSize: '0.8rem', fontWeight: 800, color: '#dc2626', textTransform: 'uppercase' }}>
                  DIQQAT!
                </span>
                <p style={{ margin: '0.2rem 0 0', fontSize: '0.75rem', color: '#dc2626', fontWeight: 550, lineHeight: 1.4 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.75rem' }}>
              <button
                ...
                style={{
                  ...
                  padding: '0.65rem 1.25rem', borderRadius: '9999px', fontSize: '0.85rem',
                  fontWeight: 700, cursor: 'pointer', transition: 'all 0.3s cubic-bezier(0.2, 0.8, 0.2, 1)'
                }}
                onMouseEnter={(e) => {
                  e.currentTarget.style.background = 'var(--bg-card-hover)';
                  e.currentTarget.style.color = 'var(--text-primary)';
              ...
              <button
                ...
                style={{
                  background: (!regenIds && !regenPasscodes) ? 'var(--text-secondary)' : 'var(--accent-hero)',
                  color: '#ffffff', border: 'none',
                  padding: '0.65rem 1.5rem', borderRadius: '9999px', fontSize: '0.85rem',
                  fontWeight: 700, cursor: (!regenIds && !regenPasscodes) ? 'not-allowed' : 'pointer',
                  boxShadow: (!regenIds && !regenPasscodes) ? 'none' : '0 8px 16px var(--accent-glow), inset 0 1px 0 rgba(255, 255, 255, 0.2)',
                  transition: 'all 0.3s cubic-bezier(0.2, 0.8, 0.2, 1)'
                }}
                onMouseEnter={(e) => { if (regenIds || regenPasscodes) e.currentTarget.style.transform = 'translateY(-1px) scale(1.02)'; }}
                onMouseLeave={(e) => { if (regenIds || regenPasscodes) e.currentTarget.style.transform = 'translateY(0) scale(1)'; }}
              >
                Tasdiqlash
              </button>
            </div>
      ...
      {assigningStudent && (
        <div style={{
          ...
          <div style={{
            ...
            <div>
              <h3 style={{ margin: '0 0 0.5rem 0', fontSize: '1.2rem', fontWeight: 800, color: 'var(--text-primary)', letterSpacing: '-0.02em' }}>
            ...
            >
              <option value="" style={{ background: 'var(--bg-card)', color: 'var(--text-primary)' }}>-- O'qituvchi tanlanmagan --</option>
              {teachers
                .filter(t => t.subject === (activeSubject === 'MATH' ? 'MATH' : 'ENG'))
                .map(t => (
                  <option key={t.id} value={t.name} style={{ background: 'var(--bg-card)', color: 'var(--text-primary)' }}>
                    {t.name}
                  </option>
                ))}
            </select>
